...
class LclsHdf5Source(HdfData):
    ...
    def __init__(self, prefix, source_path, dset_name):
        ...
        super().__init__(prefix, f'{LclsHdf5Source.DATA_ROOT}/{source_path}/{dset_name}')

    def generate_index(self, raw_file_id, h5in=None):
        time_data = np.array(h5in[f'{LclsHdf5Source.DATA_ROOT}/{self.source_path}/time'])
        
        yield from zip(gen_bunch_ids(time_data), np.arange(time_data.shape[0]))

        
class LclsHdf5Root(IndexedRoot):
    TIMING_PATH = 'Configure:0000/Run:0000/CalibCycle:0000/EvrData::DataV4/NoDetector.0:Evr.0'
    ...
    def generate_index(self, path):
        with h5py.File(path, 'r') as h5in:
            ...
            yield gen_bunch_ids(time_data), time_data['seconds'], self.data_sources, \
                dict(h5in=h5in)
