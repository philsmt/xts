...
class IterativeAbelInversion:
    ...
    def __init__(self, M_shape, row_center, col_center, r_max, r_len=1024, α_len=1024, backend=None, **kwargs):
    ...
    def integrate_pol3d(self, P1, P2):
        return 2*np.pi*(P1 * (P2 * np.sin(self.A)[None, :]).sum(axis=1) * self.R**2).sum() * self.Δr * self.Δα
    ...
    def cart2d_to_pol2d(self, M, Q1=None, Q2=None):
        ...
        if Q2 is not None:
            assert Q2.shape == (len(self.R), len(self.A)), 'Incompatible shape of Q2'
        else:
    ...
    def pol3d_to_cart2d(self, P1, P2, M=None):
        if M is not None:
            assert M.shape == (self.row_len, self.col_len), 'Incompatible shape of M'
        else:
    ...
    def M_leastsq_err(M_exp, M_cal):
        ...
        return np.sqrt( ((M_cal - M_exp)**2).sum() / (M_cal.shape[0] * M_cal.shape[1]) )
    ...
    def iterate(self, M_exp, zero_negative_values=True,
        ...
        Arguments:
            ...
            rel_err_tol (float or None): Convergence tolerance for the
                relative decrease of the optimization error or None
                for no limit.
        ...
        assert M_exp.shape == (self.row_len, self.col_len), 'Incompatible shape of M_exp'
        ...
        while True:
            ...
            try:
                ...
                if rel_err_tol is not None and rel_err_change > 0 and rel_err_change < rel_err_tol \
                        and i >= min_steps:
                    break
            ...
            if adaptive and i > 2:
                if rel_err_change > 0:
                    # If the error decreased, increase the feedback coefficients
                    # (up to their max)
                    c1 = min(c1_max, c1 * adaptive_inc)
                    c2 = min(c2_max, c2 * adaptive_inc)
                    
                else:
                    # If the error increased, decrease the feedback coefficients
                    # (down to their min)
    ...
    def step(self, c1, c2):
        try:
            D1, D2 = self.pol2d_to_pol3d(self.Q1_cal - self.Q1_exp, self.Q2_cal - self.Q2_exp)
...
def invert_abel(data, row_center, col_center, r_max=None, method=None, radial_clip=False, return_full=False, **kwargs):
    if method is None:
        ...
        try:
            ...
            raise ValueError(f'invalid inversion method \'{method}\'') from None
            
    if r_max is None:
        r_max = min([row_center, col_center, data.shape[0] - row_center, data.shape[1] - col_center])
        
    if radial_clip:
        data = apply_radial_mask(data, center=(row_center, col_center), max_radius=r_max)
