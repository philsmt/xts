...
class KaraboDataProposal(DataRoot):
    ...
    def find_run(self, target: TrainSet):
        # karabo_data requires the explicit creation of run objects. For now, this code does not support
        # TrainSets spanning multiple runs, but this should be possible with only minor performance impact
        # through proper caching

        # First, we hope to locate the run by checking the first entry against already loaded runs
...
class KaraboDataSource(DataSource):
    def __init__(self, root: KaraboDataProposal, device: Optional[str] = None, key: Optional[str] = None) -> None:
        self.root = root
        self.device = device
        self.key = key
